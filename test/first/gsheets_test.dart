...
void main() async {
  ...
  group('Testing the connectorApi ', () {
    ...
    test('inserts', () async {
      ...
      /// Tags Column
      await ws?.values
          .insertColumn(6, StatisticsSingleton().statistics?.tags.toList() ?? [], fromRow: 2);

      /// Related Links Column
      await ws?.values.insertColumn(
          5, StatisticsSingleton().statistics?.relatedLinks.toList() ?? [],
          fromRow: 2);

      /// Related People Column
      await ws?.values
          .insertColumn(4, StatisticsSingleton().statistics?.persons.toList() ?? [], fromRow: 2);
    });
  });
}
