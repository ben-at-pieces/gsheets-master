...
Future<void> toml() async {
  ...
  var languageLength = filterToml.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 33);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 33);
    }

    if (filterToml.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 33);
    } else if (filterToml.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterToml.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 33);
    }
  }
}
