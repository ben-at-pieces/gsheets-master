...
Future<void> typeScript() async {
  ...
  var languageLength = filterTypescript.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 34);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 34);
    }

    if (filterTypescript.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 34);
    } else if (filterTypescript.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterTypescript.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 34);
    }
  }
}
