...
Future<void> ruby() async {
  ...
  var languageLength = filterRuby.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 26);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 26);
    }

    if (filterRuby.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 26);
    } else if (filterRuby.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterRuby.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 26);
    }
  }
}
