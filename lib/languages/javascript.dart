...
Future<void> javascript() async {
  ...
  var languageLength = filterJavaScript.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 17);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 17);
    }

    if (filterJavaScript.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 17);
    } else if (filterJavaScript.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterJavaScript.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 17);
    }
  }
}
