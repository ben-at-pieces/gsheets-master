...
Future<void> r() async {
  ...
  var languageLength = filterR.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 25);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 25);
    }
    ...
    if (filterR.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 25);
    } else if (filterR.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterR.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 25);
    }
  }
}
