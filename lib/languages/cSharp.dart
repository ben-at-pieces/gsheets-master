...
Future<void> csharp() async {
  ...
  var languageLength = filterCSharp.length;
  ...
  if (languageLength == 0) ;
  {
    ...
    if (filterCSharp.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 6);
    } else if (filterCSharp.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterCSharp.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 6);
    }
  }
}
