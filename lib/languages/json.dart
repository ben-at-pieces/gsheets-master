...
Future<void> jason() async {
  ...
  var languageLength = filterJson.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 16);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 16);
    }
    ...
    if (filterJson.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 16);
    } else if (filterJson.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterJson.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 16);
    }
  }
}
