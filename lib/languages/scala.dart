...
Future<void> scalla() async {
  ...
  var languageLength = filterScala.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 27);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 27);
    }

    if (filterScala.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 27);
    } else if (filterScala.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterScala.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 27);
    }
  }
}
