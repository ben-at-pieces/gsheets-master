...
Future<void> swift() async {
  ...
  var languageLength = filterSwift.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 30);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 30);
    }

    if (filterSwift.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 30);
    } else if (filterSwift.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterSwift.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 30);
    }
  }
}
