...
Future<void> textRead() async {
  ...
  var languageLength = filterText.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 32);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 32);
    }
    ...
    if (filterText.isEmpty) {
      ...
      await ws?.values.insertColumn(
          3, ['${filterText.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 32);
    }
  }
}
