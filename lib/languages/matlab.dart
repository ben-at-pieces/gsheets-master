...
Future<void> matlab() async {
  ...
  var languageLength = filterMatLab.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 19);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 19);
    }

    if (filterMatLab.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 19);
    } else if (filterMatLab.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterMatLab.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 19);
    }
  }
}
