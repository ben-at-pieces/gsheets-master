...
Future<void> css() async {
  ...
  var languageLength = filterCss.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 8);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 8);
    }
    ...
    if (filterCss.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 8);
    } else if (filterCss.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterCss.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 8);
    }
  }
}
