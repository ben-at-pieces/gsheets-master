...
Future<void> python() async {
  ...
  var languageLength = filterPython.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 24);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 24);
    }

    if (filterPython.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 24);
    } else if (filterPython.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterPython.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 24);
    }
  }
}
