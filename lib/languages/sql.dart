...
Future<void> sql() async {
  ...
  var languageLength = filterSql.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 29);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 29);
    }
    ...
    if (filterSql.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 29);
    } else if (filterSql.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterSql.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 29);
    }
  }
}
