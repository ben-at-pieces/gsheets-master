...
Future<void> lua() async {
  ...
  var languageLength = filterLua.length;
  ...
  if (languageLength == 0) ;
  {
    ...
    if (filterLua.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 17);
    } else if (filterLua.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterLua.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 17);
    }
  }
}
