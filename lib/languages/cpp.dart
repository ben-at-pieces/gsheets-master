...
Future<void> cpp() async {
  ...
  var languageLength = filterPlusPlus.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 7);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 7);
    }

    if (filterPlusPlus.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 7);
    } else if (filterPlusPlus.isNotEmpty) {
      await ws?.values.insertColumn(
          3,
          [
            '${filterPlusPlus.first.original.reference?.fragment?.string?.raw}',
          ],
          fromRow: 7);
    }
  }
