...
Future<void> image() async {
  ...
  var languageLength = filterImage.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(6, ['0'], fromRow: 16);

    if (languageLength != 0) {
      await ws?.values.insertColumn(6, [languageLength], fromRow: 16);
    }
    ...
    if (filterImage.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 14);
    } else if (filterImage.isNotEmpty) {
      await ws?.values.insertColumn(7, ['bytes'], fromRow: 16);
    }
  }
}
