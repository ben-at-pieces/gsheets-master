...
Future<void> perl() async {
  ...
  var languageLength = filterPerl.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 21);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 21);
    }

    if (filterPerl.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 21);
    } else if (filterPerl.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterPerl.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 21);
    }
  }
}
