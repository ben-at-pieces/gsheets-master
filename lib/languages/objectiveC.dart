...
Future<void> objective_c() async {
  ...
  var languageLength = filterObjectiveC.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 20);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 20);
    }

    if (filterObjectiveC.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 20);
    } else if (filterObjectiveC.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterObjectiveC.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 20);
    }
  }
}
