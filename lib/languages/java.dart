...
Future<void> java() async {
  ...
  var languageLength = filterJava.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 15);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 15);
    }

    if (filterJava.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 15);
    } else if (filterJava.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterJava.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 15);
    }
  }
}
