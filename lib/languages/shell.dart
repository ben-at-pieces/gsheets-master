...
Future<void> shell() async {
  ...
  var languageLength = filterShell.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 28);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 28);
    }
    ...
    if (filterShell.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 28);
    } else if (filterShell.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterShell.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 28);
    }
  }
}
