...
Future<void> coffescript() async {
  ...
  var languageLength = filterCoffeeScript.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 6);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 6);
    }

    if (filterCoffeeScript.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 6);
    } else if (filterCoffeeScript.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterCoffeeScript.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 6);
    }
  }
}
