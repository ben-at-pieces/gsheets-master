...
Future<void> php() async {
  ...
  List<Asset> filterPhp = assetCount
      ...
      .toList();
  var languageLength = filterPhp.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 22);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 22);
    }

    if (filterPhp.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 22);
    } else if (filterPhp.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterPhp.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 22);
    }
  }
}
