...
Future<void> erlang() async {
  ...
  var languageLength = filterErlang.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 10);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 10);
    }

    if (filterErlang.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 10);
    } else if (filterErlang.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterErlang.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 10);
    }
  }
}
