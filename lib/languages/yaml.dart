...
Future<void> yaml() async {
  ...
  var languageLength = filterYaml.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 35);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 35);
    }

    if (filterYaml.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 35);
    } else if (filterYaml.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterYaml.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 35);
    }
  }
}
