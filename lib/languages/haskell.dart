...
Future<void> haskell() async {
  ...
  var languageLength = filterGo.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 12);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 12);
    }
    ...
    if (filterGo.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 12);
    } else if (filterGo.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterGo.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 12);
    }
  }
}
