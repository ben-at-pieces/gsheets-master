...
Future<void> tex() async {
  ...
  var languageLength = filterTex.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 31);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 31);
    }

    if (filterTex.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 31);
    } else if (filterTex.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterTex.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 31);
    }
  }
}
