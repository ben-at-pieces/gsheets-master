...
Future<void> powerShell() async {
  ...
  var languageLength = filterPowershell.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 23);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 23);
    }

    await ws?.values.insertColumn(1, ['POWERSHELL'], fromRow: 23);

    if (filterPowershell.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 23);
    } else if (filterPowershell.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterPowershell.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 23);
    }
  }
}
