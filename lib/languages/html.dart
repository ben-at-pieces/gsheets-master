...
Future<void> html() async {
  ...
  var languageLength = filterHtml.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 13);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 13);
    }

    await ws?.values.insertColumn(1, ['HTML'], fromRow: 13);
    ...
    if (filterHtml.isEmpty) {
      await ws?.values.insertColumn(
          ...
          fromRow: 13);
    } else if (filterHtml.isNotEmpty) {
      await ws?.values.insertColumn(
          3, ['${filterHtml.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 13);
    }
  }
}
